...
def train(detection_model, data, batch_size, epochs, save, device, num_workers):
    wandb.init(project='Object_Detection_COCO')
    ...
    num_classes = 91
    model = create_model(detection_model, num_classes=num_classes).to(device)
    ...
    for t in range(epochs):
        ...
        for batch, (inputs, targets) in enumerate(train_dataloader):
            ...
            if batch % 20 == 0:
                ...
                message = 'total loss: {:.4f}, cls loss: {:.4f}, box loss: {:.4f}  [{:>5d}/{:>5d}]'
        ...
        if best_val_map < val_mAP['map'].item():
            ...
            torch.save(model.state_dict(), f'{save}/best-object-detection-{detection_model}.pth')
            print(f'Saved PyTorch Model State to best-object-detection-{detection_model}.pth')

        wandb.log({"epoch": t + 1, "val_mAP": val_mAP['map'].item()})

    torch.save(model.state_dict(), f'{save}/last-object-detection-{detection_model}.pth')
    print(f'Saved PyTorch Model State to last-object-detection-{detection_model}.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--model', type=str, default='faster_rcnn_v2')
