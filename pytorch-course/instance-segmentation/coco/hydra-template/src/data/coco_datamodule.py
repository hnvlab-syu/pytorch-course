...
class COCODataModule(L.LightningDataModule):
    def __init__(
        ...
        if not os.path.exists(self.data_dir):
            raise FileNotFoundError(f"Data path does not exist: {self.data_dir}")
        if not os.path.exists(self.annotation_file):
            raise FileNotFoundError(f"Annotation file does not exist: {self.annotation_file}")
    ...
    def setup(self, stage=None):
        ...
        if self.mode == 'train':
            ...
            self.train_dataset = COCODataset(self.data_dir, self.annotation_file, 
                                           image_ids=train_ids, transform=self.transform)
            self.val_dataset = COCODataset(self.data_dir, self.annotation_file, 
                                         image_ids=val_ids, transform=self.transform)
            self.test_dataset = COCODataset(self.data_dir, self.annotation_file, 
                                          image_ids=test_ids, transform=self.transform)
        else:
            self.pred_dataset = [Image.open(self.data_dir).convert('RGB')]
